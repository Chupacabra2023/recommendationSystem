...
WEIGHT_CONFIGS = {
    ...
    "T02": (0.90, 0.10, 0.00, 0.00, 0.00, 0.00),  # main dominant
    "T03": (0.80, 0.20, 0.00, 0.00, 0.00, 0.00),  # main dominant
    "T04": (0.70, 0.30, 0.00, 0.00, 0.00, 0.00),  # main dominant
    "T05": (0.60, 0.40, 0.00, 0.00, 0.00, 0.00),  # main mierne
    "T06": (0.50, 0.50, 0.00, 0.00, 0.00, 0.00),  # rovnako
    "T07": (0.40, 0.60, 0.00, 0.00, 0.00, 0.00),  # sub mierne
    "T08": (0.30, 0.70, 0.00, 0.00, 0.00, 0.00),  # sub dominant
    "T09": (0.20, 0.80, 0.00, 0.00, 0.00, 0.00),  # sub dominant
    "T10": (0.00, 1.00, 0.00, 0.00, 0.00, 0.00),  # 100% sub
}
...
def sensitivity_test(user, weights, top_k=5):
    """
    Pre každý navštívený event:
    1. Získame referenčný TOP 5 s plnou históriou
    2. Skryjeme event
    3. Získame nový TOP 5
    4. PASS ak sa TOP 5 zmenil
    """
    reference_top5 = set(get_recommendations(user, weights, top_k=top_k))
    
    changes = 0
    total = len(user["visited_event_ids"])
    
    for hidden_id in user["visited_event_ids"]:
        new_top5 = set(get_recommendations(user, weights, 
                       exclude_event_id=hidden_id, top_k=top_k))
        if new_top5 != reference_top5:
            changes += 1
...
def run_evaluation():
    ...
    for cfg_name, weights in WEIGHT_CONFIGS.items():
        for user in USERS:
            changes, n = sensitivity_test(user, weights)
            hits_table[user["id"]][cfg_name] = (changes, n)
    ...
    print("\n" + "=" * len(header))
    print("  SENSITIVITY TEST - zmena TOP5 po skryti eventu")
    print("=" * len(header))
    print(header)
    print(separator)
    ...
    for cfg_name in WEIGHT_CONFIGS:
        user_scores = []
        for user in USERS:
            changes, n = hits_table[user["id"]][cfg_name]
            user_scores.append(changes / n if n else 0.0)
    ...
    print(separator)
    print(f"\nNajcitlivejsia konfig: '{best_config}'  (AVG = {fmt_pct(best_avg)})\n")

    # --- Per-user breakdown ---
    print("=" * len(header))
    print("  PER-USER DETAILS")
    print("=" * len(header))
    ...
    for user in USERS:
        ...
        for cfg_name in WEIGHT_CONFIGS:
            changes, n = hits_table[user["id"]][cfg_name]
            p = changes / n if n else 0.0
            print(f"    {cfg_name:<14}  {changes:2d}/{n}  ({fmt_pct(p)})")
