\subsubsection{Systém priateľstiev} 
  ...
  prijímanie a odmietanie, odstraňovanie priateľov a priamu komunikáciu.
                                                                                                                                                                                                                                     \iffalse                                                                                                                                                                                                                           \paragraph{Dátový model:}                                                                                                                                                                                                        
  ...
  Každé priateľstvo je uložené do Firestore kolekcie \texttt{friendships}
  s poliami \texttt{requesterId}, \texttt{receiverId}, \texttt{status}
  (\texttt{"pending"} alebo \texttt{"accepted"}) a \texttt{timestamp}.
  ...
  používateľov už vzťah existuje — duplicitné žiadosti sú zamietnuté.

  \paragraph{Odpoveď na žiadosť:}
  Pri prijatí sa stav dokumentu aktualizuje na \texttt{accepted}; pri odmietnutí
  sa dokument vymaže z Firestore aj z lokálnej pamäte.
  ...
  Odstránenie priateľa (\texttt{removeFriend()}) funguje rovnakým spôsobom
  — vyhľadá akceptované priateľstvo a vymaže ho z Firestore aj z lokálnej
  pamäte.
