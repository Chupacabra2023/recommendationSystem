\subsection{Hodnotenie udalostí}                                                                                                                                                                                                 
  ...
  Obrazovka najprv požiada o oprávnenie \texttt{CAMERA}. Stratégia
  ...
  fun checkNoShows() {
      for (event in _events.values) {
          if (event.checkInType == "none") continue
          if (uid !in event.attendees)    continue
          if (uid in event.checkedIn)     continue
          if (event.id in profile.noShowEventIds) continue
  ...
  Základom interaktivity je integračná vrstva služby \textbf{Firebase Cloud
