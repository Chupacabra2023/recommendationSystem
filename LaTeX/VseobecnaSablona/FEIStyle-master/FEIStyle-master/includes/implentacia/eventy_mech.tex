...
(história, priatelia, správy) nemažú. Administrátor môže priamo vo Firebase Console
vidieť dôvod (\texttt{banReason}) a čas (\texttt{banTimestamp}) banu spolu
s nahláseniami v kolekcii \texttt{reports/} a manuálne rozhodnúť o odbanovaní
(\texttt{banned: false}) alebo permanentnom vymazaní účtu.
...
suspend fun recordNoShow(userId: String, eventId: String) {
    ...
    if (shouldBan) {
        updates["banned"]       = true
        updates["banReason"]    = "no_show"
        updates["banTimestamp"] = now
        cancelPendingInvitations(userId)
    }
...
Pri každom zablokovaní účtu (či už z dôvodu no-show alebo nahlásení) sa
automaticky vymažú všetky čakajúce pozvánky adresované zabanovanému
používateľovi, aby ostatní nedostávali odpoveď od zablokovaného účtu:
